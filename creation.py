...
def skills_filter(word):
    tag = nltk.pos_tag([word], lang='rus')[0][1]
    return tag.startswith('A') or tag == 'NONLEX'

def add_series_to_db(series, name, db_connection):
    cursor = db_connection.selector()
...
quality_errors = ['вредный', 'личный', 'новый', 'высокий', 'электронный', 'большой', 'научный', 'государственный',
...
attribute = 'jobTitle'
add_series_to_db(items_stem_top(attribute, works[attribute].dropna(),
                                lambda word: True,
...
attribute = 'skills'
add_series_to_db(items_stem_top(attribute, [clean_html(skill) for skill in works.skills.dropna()],
                                skills_filter,
...
attribute = 'otherInfo'
add_series_to_db(items_stem_top(attribute, [clean_html(info) for info in works.otherInfo.dropna()],
                                lambda w: 'analysis' in w and len(w['analysis']) > 0 and w['analysis'][0]['gr'].startswith(
                             'A='),
                                lambda s: s not in quality_errors, 54, False)[attribute], attribute, connection)